...
const likePost = async (req, res) => {
  ...
  try {
    const postlike = await Posts.findByIdAndUpdate(postToLikeId, {
      $push: { likes: { userID: likerId } },
    });
...
const unLikePost = async (req, res) => {
  ...
  try {
    const postlike = await Posts.findByIdAndUpdate(postToUnLikeId, {
      $pull: { likes: { userID: unlikerId } },
    });
...
const commentPost = async (req, res) => {
  ...
  try {
    const post = await Posts.findByIdAndUpdate(postToCommentId, {
      $push: { comments: { userID: id, comment: comment } },
    });
...
const deleteComment = async (req, res) => {
  ...
  console.log(deleteID._id, postId);
  try {
    const commentDelete = await Posts.findByIdAndUpdate(
      postId,
      { $pull: { comments: { _id: deleteID } } },
      { new: true }
    );

    res.status(200).json({ mssg: 'comment deleted', commentDelete });
  } catch (error) {
    res.status(401).json({ error: error.message });
  }
};

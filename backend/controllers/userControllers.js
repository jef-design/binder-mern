...
const generateToken = (res, _id) => {
    const jwtExpires = 3600
     const createdtoken = jwt.sign({_id}, process.env.JWT_SECRET, {expiresIn: jwtExpires})
    ...
    if (req.cookies?.jwt) {
        ...
        (err, decoded) => {
            if (err) {
                ...
                const refreshTokenJwt = jwt.sign({_id}, process.env.JWT_SECRET, {expiresIn: '2d'});
...
const getUserPost = async (req, res) => {
    ...
    const posts = await Posts.find({userID: user}).sort({createdAt: -1}).populate({
        path: 'userID',
...
module.exports = {signUpUser, logInUser, logOutUser, getUserDetails, upDateUser, followUser,UnfollowUser,getUserPost,getUsers, refreshToken}
